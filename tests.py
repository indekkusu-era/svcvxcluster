...
if __name__ == "__main__":
    X, y = load_semisphere(1.0, 1.4, 1.6, 2.0, 200000) # load_large(500000, 10)
    ...
    clust = SvCvxCluster(10, alpha=0.4, alpha_prime=1, 
                         ...
                         solver_config=SolverConfig(gamma=0.75, mu_update_tol=1, 
                                            ...
                                            cgtol_tau=1/2, cgtol_default=1e-5, parallel=True
                                        ),
                                    solver=thread_sv_cvx_cluster)
    t = perf_counter_ns()
    clust.fit(X, warm_start=False)
    print("threading SSNAL:", (perf_counter_ns() - t) / 1e9)
